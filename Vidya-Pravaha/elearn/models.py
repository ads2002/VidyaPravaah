...
class Parent(models.Model):
    user = models.OneToOneField(User,on_delete=models.CASCADE)
    is_parent = models.BooleanField(default=False)
    learner_linked = models.ForeignKey(Learner,on_delete=models.CASCADE, null= True,blank=True)



class LearnerParentConnection(models.Model):
    # student = models.ForeignKey(Learner, on_delete=models.CASCADE)
    parent = models.ForeignKey(Parent, on_delete=models.CASCADE)
    # user_id = models.CharField(max_length=100)
    learner= models.ForeignKey(Learner,on_delete=models.CASCADE, related_name='parents')

    def __str__(self):
        return f"Parent:n{self.parent.username}, Learner: {self.learner.name}"
